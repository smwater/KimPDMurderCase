...
void update_ending(void)
{
	...
	Fade_In(&data->BackGround, data->Alpha);
	Fade_Out(&data->BackGround, data->Alpha);

	Scale_Down(&data->BackGround);
	Scale_Up(&data->BackGround);

	//Move_Left(&data->BackGround, data->X, data->Y);
	//Move_Right(&data->BackGround, data->X, data->Y);
}
